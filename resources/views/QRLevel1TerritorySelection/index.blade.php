                                ...
                                <table id="datatables" class="table table-striped table-no-bordered table-hover" cellspacing="0" width="100%" style="width:100%">
                                    ...
                                    @foreach ($Level1Territories as $Level1Territory)
                                        <tr id="row_{{$Level1Territory->Level1TerritoryId}}">
                                            ...
                                            <td>
                                                @if (($Level1Territory->IsQRFoundINL1Ter == 0 || $Level1Territory->IsQRFoundINL2Ter != 0))
                                                    @if($CampaignLevel1TerritoryId == $Level1Territory->CampaignLevel1TerritoryId || $CampaignLevel1TerritoryId == 0) 
                                                        <a id="2TerritoryLink{{$Level1Territory->Level1TerritoryId}}" href="QRLevel2TerritorySelection?id={{$Level1Territory->CampaignLevel1TerritoryId}}" style="<?=empty($Level1Territory->TargetURL)?"display: block;":"display: none;";?>">Add / Remove</a>
                                                    @endif 
                                                @endif
                                            </td>
                                            <td class="text-center">
                                                @if ($Level1Territory->IsQRFoundINL2Ter == 0)
                                                    @if($CampaignLevel1TerritoryId == $Level1Territory->CampaignLevel1TerritoryId || $CampaignLevel1TerritoryId == 0) 
                                                        <a href="#" data-toggle="modal" data-target=".qrmodal" onclick="ViewCampaignInQRCodes({{$B2BCustomerId}},{{$CampaignId}},{{$Level1Territory->CampaignLevel1TerritoryId}})"> <?=($Level1Territory->QRCodeFileName!=NULL)?"(".$Level1Territory->QRCodeFileName.") <a class='btn btn-simple btn-danger btn-icon' onclick='OnRemoveQRCodeInCampaign(".$Level1Territory->QRCodeId.",".$CampaignId.",".$Level1Territory->CampaignLevel1TerritoryId.")' href='#''><i class='material-icons dp48'>remove_circle</i></a>":'Add';?></a>
                                                      @endif  
                                                  @endif 
                                            </td>
                                            <td class="text-center">
